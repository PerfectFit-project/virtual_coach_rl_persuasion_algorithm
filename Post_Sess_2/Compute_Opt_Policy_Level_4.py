...
# This is so that the number of samples does not change.
# Our hypothesis is that using similarity-weighting is useful - this does not
# necessarily mean that we use samples from the same person.
# But this is good to keep in mind for the discussion of the paper possibly.
for p1 in range(num_assigned):
    
    print("Person index from Group 4 in assignment.csv:", p1)
    
    # Get index of traits for this person
    trait_index_p1 = traits_ids.index(user_ids_assigned[p1])
    ...
    opt_policies[user_ids_assigned[p1]] = opt_policy
